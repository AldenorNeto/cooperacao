// Gerenciador de Gráficos - Histórico de Fitness
// Plota evolução do fitness ao longo das gerações
...
const ChartManager = {
  ...
  addFitnessPoint(generation, fitness, delivered) {
    this.fitnessHistory.push({ generation, fitness, delivered, timestamp: Date.now() });
  ...
  draw() {
    ...
    // Desenha eixos
    this.drawAxes(ctx, padding, width, height, minGen, maxGen, adjustedMinFit, adjustedMaxFit);
    
    // Desenha linha do fitness
    this.drawFitnessLine(ctx, padding, chartWidth, chartHeight, minGen, maxGen, adjustedMinFit, adjustedMaxFit);
    
    // Desenha pontos de entrega
    this.drawDeliveryPoints(ctx, padding, chartWidth, chartHeight, minGen, maxGen, adjustedMinFit, adjustedMaxFit);
  },

  drawAxes(ctx, padding, width, height, minGen, maxGen, minFit, maxFit) {
    ctx.strokeStyle = '#666';
    ctx.lineWidth = 1;
  ...
  drawFitnessLine(ctx, padding, chartWidth, chartHeight, minGen, maxGen, minFit, maxFit) {
    ...
    for (let i = 0; i < this.fitnessHistory.length; i++) {
      ...
      const y = padding + chartHeight - ((point.fitness - minFit) / (maxFit - minFit)) * chartHeight;
  ...
  drawDeliveryPoints(ctx, padding, chartWidth, chartHeight, minGen, maxGen, minFit, maxFit) {
  ...
  saveToStorage() {
    try {
      localStorage.setItem(this.storageKey, JSON.stringify(this.fitnessHistory));
    } catch (e) {
