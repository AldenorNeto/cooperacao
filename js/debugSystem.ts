const DebugSystemImpl = {
  ...
  updateAgentInfo(agent: any): void {
    ...
    if (elements.agentPos) elements.agentPos.textContent = `${agent.x.toFixed(1)}, ${agent.y.toFixed(1)}`;
    if (elements.agentAngle) elements.agentAngle.textContent = `${(agent.a * 180 / Math.PI).toFixed(1)}°`;
    ...
    if (elements.agentCarry) elements.agentCarry.textContent = agent.carry ? 'true' : 'false';
    if (elements.agentFitness) elements.agentFitness.textContent = agent.fitness.toFixed(2);
  },

  updateSensorInfo(sensorData: any[]): void {
    if (!this.isActive) return;
  ...
  drawNeuralNetwork(genome: any, inputs: number[], outputs: number[]): void {
    if (!this.isActive) return;
    ...
    for (let i = 0; i < layers.input.count; i++) {
      for (let h = 0; h < layers.hidden.count; h++) {
        ...
        ctx.strokeStyle = weight > 0 ? `rgba(0, 255, 0, ${alpha * 0.5})` : `rgba(255, 0, 0, ${alpha * 0.5})`;
  ...
  update(agent: any, stepResult: any): void {
    ...
    if (stepResult) {
      ...
      this.drawNeuralNetwork(agent.genome, stepResult.inputs, stepResult.outputs);
    }
  }
};

if (typeof window !== "undefined") {
  (window as any).DebugSystem = DebugSystemImpl;
}